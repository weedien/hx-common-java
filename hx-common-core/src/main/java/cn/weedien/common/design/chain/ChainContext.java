package cn.weedien.common.design.chain;
...
public final class ChainContext<T> implements CommandLineRunner {
    ...
    private final Map<String, List<ChainHandler<T>>> chainHandlerContainer = new HashMap<>();
    ...
    public void handle(String mark, T param) {
        List<ChainHandler<T>> handlers = chainHandlerContainer.get(mark);
    ...
    public void run(String... args) {
        ...
        chainFilterMap.forEach((beanName, bean) -> {
            List<ChainHandler<T>> chainHandlers = chainHandlerContainer.getOrDefault(bean.mark(), new ArrayList<>());
            chainHandlers.add(bean);
            // 排序
            List<ChainHandler<T>> actualAbstractChainHandlers = chainHandlers.stream()
